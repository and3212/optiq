...
void move1(std::ofstream &stepFile, std::ofstream &timeFile, short movementType) {
    ...
    if(movementType == RAPID){
        double theta1Initial = findTheta(1, 0, (short)0, beta, beta1, R);
        double theta2Initial = findTheta(2, 0, (short)0, beta, beta1, R);
        double theta1Final = findTheta(1, 60, (short)0, beta, beta1, R);
        double theta2Final = findTheta(2, 60, (short)0, beta, beta1, R);

        double deltaTheta1 = theta1Final - theta1Initial;
        double deltaTheta2 = theta2Final - theta2Initial;
        ...
        remainder1 = numSteps1 - trunc(numSteps1);
        remainder2 = numSteps2 - trunc(numSteps2);
...
//        }
//
//        while (numSteps1 > 0 && numSteps2 > 0) {
//            stepFile << printString;
//            timeFile << "1\n";
//            numSteps1--;
//            numSteps2--;
//        }
    ...
    segmentDistance = sqrt((xCoord1 - xCoord2) * (xCoord1 - xCoord2) + (yCoord1 - yCoord2) * (yCoord1 - yCoord2))/SEGMENTS;
    ...
    if (movementType != LINE) {
        ...
        segmentDistance = beta*R/SEGMENTS;
    ...
    //totalDistance += fabs(segmentDistance*SEGMENTS);
    //timeFile << totalDistance << "\n";
    ...
    for (int i = 0; i < SEGMENTS; i++) {
        ...
        //TODO add this to code
        if((numSteps1 + numSteps2) != 0) {
            stepSpeedD = ((fabs(segmentDistance) / speed) / (numSteps1 + numSteps2))*1000;
            stepSpeed = (int)ceil(((fabs(segmentDistance) / speed) / (numSteps1 + numSteps2))*1000);
        }
        else {
            stepSpeed = -1;
        }
